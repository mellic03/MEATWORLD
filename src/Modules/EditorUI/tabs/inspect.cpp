...
static void
tab_inspect_CS( idk::RenderEngine &ren, Transform_CS &CS, int object_id )
{
    EditorUI::dragVec3(
        "Position",
...
static void
tab_inspect_CS( idk::RenderEngine &ren, Model_CS &CS, int object_id )
{
...
tab_inspect_component( idk::EngineAPI &api, int object_id, idk::ComponentSystem *CS )
{
    ...
    {
        tab_inspect_CS(ren, *dynamic_cast<Transform_CS *>(CS), object_id);
    }

    if (CS == &engine.getCS<Model_CS>())
    {
        tab_inspect_CS(ren, *dynamic_cast<Model_CS *>(CS), object_id);
    }

    if (CS == &engine.getCS<idkg::Terrain_CS>())
    {
        tab_inspect_CS(ren, *dynamic_cast<idkg::Terrain_CS *>(CS), object_id);
    }
